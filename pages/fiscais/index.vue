<template>
    <v-row justify="center" class="mb-4">
        ...
        <fiscalCadastro v-if="exibCadastro" :open="exibCadastro" @close="exibCadastro = false" @cancelar="cancelar"
...
<script>
import { fiscalModel } from '~/models/FiscalModel'
export default {
    async asyncData({ $axios }) {
        let listagem = []
        try {
            const resposta = await $axios.$get('/pessoas/?tipo=4')
            if (!resposta?.erro) {
    ...
    methods: {
        ...
        async atualizarListagem() {
            try {
                const resposta = await this.$axios.$get('/pessoas/?tipo=4')
                if (!resposta?.erro) {
        ...
        async exibirItem(item) {
            const { id } = item
            try {
                // this.payload = fiscalModel(await this.$axios.$get(`/pessoa/${id}`))
                const pessoa = await this.$axios.$get(`/pessoa/${id}`)
                pessoa.usuario = (pessoa.usuario[0] || null)
                this.payload = fiscalModel(pessoa)
                this.exibCadastro = true
                this.isEdit = true
            } catch (error) {
                console.log(error);
