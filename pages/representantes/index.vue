...
export default {
    async asyncData({ $axios }) {
        let listagem = []
        try {
            const resposta = await $axios.$get('/representantes')
            if (!resposta?.erro) {
    ...
    methods: {
        ...
        novoItem() {
            this.payload = representanteModel()
            this.isEdit = false
        ...
        async atualizarListagem() {
            try {
                const resposta = await this.$axios.$get('/representantes')
                if (!resposta?.erro) {
        ...
        async exibirItem(item) {
            const { id } = item
            try {
                const payload = await this.$axios.$get(`/representante/${id}`)
                this.payload = representanteModel(payload)
                this.exibCadastro = true
                this.isEdit = true
            } catch (error) {
                console.log(error);
